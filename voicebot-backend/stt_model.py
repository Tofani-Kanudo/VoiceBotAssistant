...
def transcribe_audio(audio_data: bytes) -> str:
    try:
        ...
        raw_audio = np.array(audio_segment.get_array_of_samples()).astype(np.float32) / 32768.0
        logger.debug("Audio pre-processing complete")
        ...
        log_error_with_traceback(logger, f"Error during transcription: {str(e)}")
        raise
